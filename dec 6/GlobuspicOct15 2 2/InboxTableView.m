...
@synthesize delete, objects,refreshControl,message,followingActivitiesQuery,photosFromCurrentUserQuery,query,query1,allObjects,checkTypeAry,fromUserArray,toUserArray,msgAry;
...
- (void)viewDidLoad
{
   ...
   // [self displayBadges];
    ...
    self.msgAry=[[NSMutableArray alloc]init];
        ...
        objects=(NSMutableArray*)[self.query findObjects];
            ...
            if (self.objects.count == 0) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    self.noDataView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 40, self.view.frame.size.width, 80)];
                    self.noDataView.image = [UIImage imageNamed:@"no_data.png"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.checkTypeAry[indexPath.row]  isEqualToString: @"image"]) {
        ...
        message = [self.msgAry objectAtIndex:indexPath.row];
        
        self.fullPhoto.arrUser=arrUser;
        self.fullPhoto.message=message;
        NSLog(@"self.fullPhoto.message is %@", self.fullPhoto.message);
