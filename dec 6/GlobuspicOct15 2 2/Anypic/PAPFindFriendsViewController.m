...
-(void)parseFollowingUser{
    array=[[NSMutableArray alloc]init];
    
    PFQuery *parsefollowerquery =[PFQuery queryWithClassName:kPAPActivityClassKey];
    [parsefollowerquery whereKey:kPAPActivityFromUserKey equalTo:[PFUser currentUser]];
    [parsefollowerquery whereKey:kPAPActivityTypeKey equalTo:kPAPActivityTypeFollow];
//    [parsefollowerquery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
    
    @try {
         NSArray *objects=[parsefollowerquery findObjects];
        
        for (PFObject *obj in objects) {
            PFUser *usr=obj[kPAPActivityToUserKey];
                //            [usr fetchInBackgroundWithBlock:^(PFObject *ob, NSError *error) {
            PFObject *ob  = [usr fetchIfNeeded];
            [array addObject: ob[kPAPUserFacebookIDKey]];
                //            }];
...
//    if (array.count == arr.count) {
        [parseEmployeeQuery whereKey:kPAPUserFacebookIDKey containedIn:array];
