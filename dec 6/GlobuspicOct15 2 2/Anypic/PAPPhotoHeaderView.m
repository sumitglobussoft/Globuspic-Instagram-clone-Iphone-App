...
- (id)initWithFrame:(CGRect)frame buttons:(PAPPhotoHeaderButtons)otherButtons {
    ...
    if (self) {
        ...
        // timestamp
        self.timestampLabel = [[UILabel alloc] initWithFrame:CGRectMake( 50.0f, 22.0f, containerView.bounds.size.width - 49.0f - 72.0f, 18.0f)];
...
- (void)setPhoto:(PFObject *)aPhoto {
    ...
    self.place=[self.photo objectForKey:@"place"];
    self.PlaceLabel.text=self.place;
//    NSLog(@"Place %@",self.place);
    
    NSArray * tagListArr=[self.photo objectForKey:@"TagList"];
    NSString * str=[NSString stringWithFormat:@"%lu",(unsigned long)tagListArr.count];
    [self.tagButton setTitle:str forState:UIControlStateNormal];
