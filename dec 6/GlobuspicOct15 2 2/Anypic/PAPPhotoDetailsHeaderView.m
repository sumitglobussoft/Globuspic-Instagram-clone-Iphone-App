...
@synthesize photo,tapRecognizer,toggle;
...
-(void)hideTag{
    for (UIView *v in [self.photoImageView subviews]){
        if ([v isKindOfClass:[CustomTagLabel class]]) {
            v.hidden = YES;
        }
    }
}
...
//    [self.tapRecognizer setDelegate:self];
    ...
    NSString *str=[self.photo objectForKey:@"Type"];
    if ([str isEqualToString:@"Video"]) {
        
      PFFile *thumFile = [self.photo objectForKey:@"thumbnail"];
        [thumFile getDataInBackgroundWithBlock:^(NSData *imageData, NSError *error) {
            if (!error) {
        ...
        UIWebView *webView=[[UIWebView alloc]initWithFrame:CGRectMake(mainImageX, mainImageY, mainImageWidth, mainImageHeight)];
        webView.backgroundColor=[UIColor colorWithPatternImage:image1];
        [self addSubview:webView];
        NSURLRequest *nsrequest=[NSURLRequest requestWithURL:[NSURL URLWithString:imageFile.url]];
        [webView loadRequest:nsrequest];
         ...
         self.photoImageView.image = [UIImage imageNamed:@"PlaceholderPhoto1.png"];
         self.photoImageView.backgroundColor = [UIColor blackColor];
         self.photoImageView.contentMode = UIViewContentModeScaleAspectFit;
        self.photoImageView.file = imageFile;
        [self.photoImageView loadInBackground];
        for (NSDictionary *dict in [self.photo objectForKey:@"TagList"]) {
            
            CustomTagLabel *label = [[CustomTagLabel alloc]initWithFrame:CGRectMake([[dict valueForKey:@"Xposition" ]floatValue], [[dict valueForKey:@"Yposition" ]floatValue], 90, 30)];
            
            NSString * fname = [dict valueForKey:@"name"] ;
