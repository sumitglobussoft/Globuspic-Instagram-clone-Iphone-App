...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    if (indexPath.section == self.objects.count) {
       ...
       PAPPhotoCell * cell = (PAPPhotoCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        ...
        cell.imageView.image = [UIImage imageNamed:@"PlaceholderPhoto1.png"];
        
//        UIView *view1=[[UIView alloc] initWithFrame:CGRectMake(20, 40, 50, 20)];
//        view1.backgroundColor=[UIColor greenColor];
//        [cell.imageView addSubview:view1];
        
        
        /*array=[[NSMutableArray alloc] init];
        array=object[@"TagList"];
        NSLog(@"array %@",array);
        ...
        for (NSDictionary *dict in array) {
            label=[[UILabel alloc] initWithFrame:CGRectMake([[dict valueForKey:@"Xposition" ]floatValue], [[dict valueForKey:@"Yposition" ]floatValue], 100, 30)];
...
//            label.hidden=YES;
       ...
       button.frame=CGRectMake(20, cell1.imageView.frame.size.height-60, 50, 50);
           button.tag=indexPath.section;
           [button addTarget:self action:@selector(tagButton:) forControlEvents:UIControlEventTouchUpInside];
           [button setImage:[UIImage imageNamed:@"tagpicSelected.png"] forState:UIControlStateNormal];
           
       [cell1.contentView addSubview:button];*/
       //}
        
        if (object) {
          NSString *typeStr =object[@"Type"];
//
...
-(void)tagButton:(UIButton *)sender{
    ...
    array=[[NSMutableArray alloc] init];
    //array=object[@"TagList"];
    NSLog(@"sender section %ld",(long)sender.tag);
    PFObject * obj=[self.objects objectAtIndex:sender.tag];
    NSLog(@"Pfobject Obj %@",obj);
    array=obj[@"TagList"];
    ...
    for (NSDictionary *dict in array) {
        ...
        label.textColor=[UIColor whiteColor];
        //            label.hidden=YES;
        [cell1.imageView addSubview:label];
